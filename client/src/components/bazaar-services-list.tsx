          ...
          {sortedServices.map((service, index) => {
            ...
            return (
              <article key={`${service.resource}-${index}`} className="w-full rounded-2xl border border-border/70 bg-muted/10 p-6">
                ...
                {providerNote ? (
                  <p className="mt-3 text-xs italic text-muted-foreground">{providerNote}</p>
                ) : null}
                ...
                {accepts.length > 0 || metadataEntries.length > 0 ? (
                  <Accordion type="multiple" className="mt-4 space-y-2 text-sm">
                    {accepts.map((payment, paymentIndex) => {
                      ...
                      return (
                        <AccordionItem
                          key={`${payment.asset}-${paymentIndex}`}
                        ...
                        >
                          ...
                          <AccordionContent className="px-3 pb-3 pt-0">
                            <div className="space-y-2 pt-2 text-[0.7rem] text-muted-foreground">
                              ...
                              {payment.mimeType ? (
                                <FieldRow label="MIME type" value={payment.mimeType} className="text-xs" />
                              ) : null}

                              {payment.maxAmountRequired ? (
                                <FieldRow
                                  label="Max amount"
                                  value={formatMaxAmount(payment.maxAmountRequired)}
                                  className="text-xs"
                                />
                              ) : null}
                              ...
                              {payment.maxTimeoutSeconds ? (
                                <FieldRow
                                  label="Timeout"
                                  value={`${payment.maxTimeoutSeconds} seconds`}
                                  className="text-xs"
                                />
                              ) : null}

                              {inputSchema?.method ? (
                                <FieldRow label="Method" value={inputSchema.method} className="text-xs" />
                              ) : null}

                              {inputSchema?.type ? (
                                <FieldRow label="Type" value={inputSchema.type} className="text-xs" />
                              ) : null}
                              ...
                              {inputSchema?.discoverable !== undefined ? (
                                <FieldRow
                                  label="Discoverable"
                                  value={inputSchema.discoverable ? "yes" : "no"}
                                  className="text-xs"
                                />
                              ) : null}
